...
export function getMainColors(data: ImgPixels) {
  ...
  data.forEach((pixel) => {
    ...
    while (Node.leafNum > mainColorNumber * 4) reduceTree();
  });
  ...
  const result = Object.entries(record)
    .sort((a, b) => b[1] - a[1])
    .slice(0, mainColorNumber);

  console.log("# colors", { data, record });
...
function colorsStats(node: Node, record: Record<string, number>) {
  if (node.isLeaf) {
    const r = (~~(node.r / node.childrenCount)).toString(16).padStart(2, "0");
    const g = (~~(node.g / node.childrenCount)).toString(16).padStart(2, "0");
    const b = (~~(node.b / node.childrenCount)).toString(16).padStart(2, "0");
