...
class PlayState {
  ...
  /** 刷新随机列表 */
  resetRandList() {
    const list = this.playList.map((m) => m.id);
    ...
    this._randPlayList = list;
  }

  /** 根据 id 将 playList 中该歌曲的信息补全 */
  async populateMusic(id: number) {
    const targetIndex = this.playList.findIndex((m) => m.id === id);

    // 有专辑信息则认为有全部信息, 不需要额外获取
    if (this.playList[targetIndex].al) return;
  ...
  switchMusic(direction: "next" | "prev") {
    if (this.playMode === PlayMode.RAND) {
      ...
      // 一方面保证用户切上一首时可以保证可靠性
      // 其次也可使用户短时间内听不到重复的歌
      const curIndex = this._randPlayList.findIndex(
        (id) => id === this.curMusic.id
      );
      const nextInd =
        (curIndex +
          (direction === "next" ? 1 : -1) +
          this._randPlayList.length) %
        this._randPlayList.length;
